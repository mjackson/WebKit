#!/usr/bin/env bash
# Round-1 batch triage + dedup of Fuzzilli crashes.
#
# For every crash .js across all r1 storage dirs:
#   1. Extract the per-crash TARGET ARGS from the Fuzzilli header.
#   2. Re-run under the current Fuzz jsc (ASAN), up to 3 attempts (thread bugs
#      are flaky).
#   3. Derive a signature: crash kind + top in-JSC frames (ASAN interceptors,
#      libc, allocator frames stripped).
#   4. Dedupe by signature; copy one repro per unique signature to
#      Tools/threads/fuzz/crashes/r1/<id>.js with the captured ASAN report
#      alongside as <id>.stderr.
#
# Output: Tools/threads/fuzz/crashes/r1/INDEX.tsv (sig -> repro path),
#         WebKitBuild/Fuzz/triage-r1/all.tsv (every crash file -> sig).
set -uo pipefail

REPO=/root/WebKit
JSC="$REPO/WebKitBuild/Fuzz/bin/jsc"
OUT="$REPO/Tools/threads/fuzz/crashes/r1"
WORK="$REPO/WebKitBuild/Fuzz/triage-r1"
PAR=${PAR:-12}
...
mapfile -t FILES < <(ls "$REPO"/WebKitBuild/Fuzz/fuzzilli-storage/crashes/*.js \
                        "$REPO"/WebKitBuild/Fuzz/fuzzilli-storage-B/crashes/*.js \
                        "$REPO"/WebKitBuild/Fuzz/fuzzilli-storage-C/crashes/*.js 2>/dev/null | sort)

echo "Triaging ${#FILES[@]} crash files (PAR=$PAR)..." >&2
