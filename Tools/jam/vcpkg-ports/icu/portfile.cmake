# Jam overlay of microsoft/vcpkg ports/icu at
# 43ea817e84600347895efc196e2bb91162e9e679. Keep this file in lockstep with
# that pin. CI copies the pin's patches and cmake wrapper next to this file.
# The Jam delta is the data-zip + ICU_DATA_FILTER_FILE block after extract
# and the filtered-list gate.

if(NOT EXISTS "${CMAKE_CURRENT_LIST_DIR}/disable-static-prefix.patch")
    message(FATAL_ERROR "vcpkg ICU patches must be copied next to this overlay portfile")
endif()
...
vcpkg_download_distfile(
    ...
    SHA512 04A49455E1489030C520A4BFD2664FA2171E7938D08F2EEC89BA59A7B6D118CEAAB6EC5A096E40D9085A0895D91CE225245
)
...
# Same data recipe as .github/workflows/jam-webkit.yml "Build filtered ICU":
# drop the prebuilt archive, unpack the full data zip, then filter.
if(NOT DEFINED ENV{JAM_ICU_DATA_ZIP} OR NOT EXISTS "$ENV{JAM_ICU_DATA_ZIP}")
    message(FATAL_ERROR "JAM_ICU_DATA_ZIP must point at icu4c-${VERSION}-data.zip")
endif()
file(REMOVE "${SOURCE_PATH}/source/data/in/icudt78l.dat")
execute_process(
    COMMAND "${CMAKE_COMMAND}" -E tar x "$ENV{JAM_ICU_DATA_ZIP}"
    WORKING_DIRECTORY "${SOURCE_PATH}/source"
    COMMAND_ERROR_IS_FATAL ANY
)
if(NOT DEFINED ENV{JAM_ICU_FILTER_FILE} OR NOT EXISTS "$ENV{JAM_ICU_FILTER_FILE}")
    message(FATAL_ERROR "JAM_ICU_FILTER_FILE must point at icu-data-filter.json")
endif()
set(ENV{ICU_DATA_FILTER_FILE} "$ENV{JAM_ICU_FILTER_FILE}")
...
set(_jam_icu_lst "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/data/out/tmp/icudata.lst")
if(NOT EXISTS "${_jam_icu_lst}")
    file(GLOB_RECURSE _jam_icu_lsts "${CURRENT_BUILDTREES_DIR}/*/data/out/tmp/icudata.lst")
    list(LENGTH _jam_icu_lsts _jam_icu_lst_count)
    if(NOT _jam_icu_lst_count EQUAL 1)
        message(FATAL_ERROR "ICU data list missing after filtered build: ${_jam_icu_lst}")
    endif()
    list(GET _jam_icu_lsts 0 _jam_icu_lst)
endif()
file(READ "${_jam_icu_lst}" _jam_icu_lst_txt)
if(_jam_icu_lst_txt MATCHES "(^|\n|/)([^/\n]+\\.(cnv|spp)|confusables\\.cfu|unames\\.icu|translit/|rbnf/)")
    message(FATAL_ERROR "Excluded ICU data survived filtering (see ${_jam_icu_lst})")
endif()
