...
RUN --mount=type=tmpfs,target=/icu \
    if [ -z "$ENABLE_SANITIZERS" ] && [ "$WEBKIT_RELEASE_TYPE" = "Release" -o "$WEBKIT_RELEASE_TYPE" = "MinSizeRel" -o "$WEBKIT_RELEASE_TYPE" = "RelWithDebInfo" ]; then \
        export CFLAGS="$CFLAGS -Os -std=c17 -DNDEBUG $LTO_FLAG"; \
        export CXXFLAGS="$CXXFLAGS -Os -DNDEBUG -DUCONFIG_NO_LEGACY_CONVERSION=1 -std=c++20 -fno-exceptions $LTO_FLAG -fno-c++-static-destructors "; \
    else \
...
RUN --mount=type=tmpfs,target=/webkitbuild \
    if [ -z "$ENABLE_SANITIZERS" ] && [ "$WEBKIT_RELEASE_TYPE" = "Release" -o "$WEBKIT_RELEASE_TYPE" = "MinSizeRel" -o "$WEBKIT_RELEASE_TYPE" = "RelWithDebInfo" ]; then \
        export CFLAGS="$CFLAGS -DNDEBUG $LTO_FLAG -ffile-prefix-map=/webkit/Source=vendor/WebKit/Source  -ffile-prefix-map=/webkitbuild/=. "; \
        export CXXFLAGS="$CXXFLAGS -DNDEBUG $LTO_FLAG -fno-c++-static-destructors -ffile-prefix-map=/webkit/Source=vendor/WebKit/Source -ffile-prefix-map=/webkitbuild/=. "; \
    else \
